---
title: "Hobbyprojekte"
featured_image: '/images/banner_blackboard2.jpg'
omit_header_text: true
date: 2020-06-01T16:49:58+01:00
draft: false
video: "german"
---

Flatten the curve
-----------------

Um die ["flatten the curve"-Strategie](https://www.br.de/nachrichten/wissen/warum-es-gut-ist-die-coronavirus-ausbreitung-zu-verlangsamen,RsqD1Hj) gegen COVID-19 zu veranschaulichen, habe ich diese kleine Simulation in Python programmiert. Blaue Punkte stehen für gesunde Individuen, rote Punkte für infizierte. Wenn ein blauer Punkt einem roten zunahe kommt, wird er infiziert (und damit infektiös). Nach einer bestimmten Zeit genesen infizierte Punkte von alleine. Man sieht, dass diese einfache Dynamik global zu einer Art Welle führt.

Der Code ist hier: https://github.com/frank-roesler/Files/blob/master/Flatten_curve.py 
*  	Erstes Video: Schlechte Situation mit hohem Peak;
	{{< youtube sTvhKj-CLAU >}}
	
* 	Zweites Video: Gute Situation: Der mittlere Abstand der Punkte ist größer (siehe Achsenbeschriftung) und die Kurve dadurch flacher.
{{< youtube 6HUuNNzwgrU >}}

